...
- `export-disabled` keeps **コピー** / **ファイルに保存** disabled while status is not `ready`.

## How to get to it (user POV)

- Finish a meeting so `/meetings/<id>/document` is ready (preview: **おためし** → **おわる** → **はい、おわる**).
- Use the header-adjacent **コピー** and **ファイルに保存**.
...
- **Copy.** `page.getByRole("button", { name: "コピー" }).click()`. Toast `コピーしました` is visible. `navigator.clipboard.readText()` contains `# <meeting title>` (preview title from the name field).
- **Download.** `const download = page.waitForEvent("download")` then `page.getByRole("button", { name: "ファイルに保存" }).click()` (role name `保存` also matches). Suggested filename is `requirements-<meetingId>.md` where `meetingId` is `pathname.split("/")[2]`. Toast `ファイルに保存しました` is visible.
...
- **Proof.** Screenshot ready まとめ with **コピー** / **ファイルに保存** enabled, then the post-copy toast or the edited preview heading. Video: **コピー** (or **なおす** → edit → **見る**). Keep the downloaded `.md` next to the screenshots when download is driven. Record feature id `export-markdown`.

The spec `会議終了からまとめの確認・編集・書き出しまで通る` drives copy, download, and edit on `:3217`.

## Gotchas

- Whisper cards also have a **コピー** button (question text). On the document page use the export **コピー** in the action row, not a whisper.
