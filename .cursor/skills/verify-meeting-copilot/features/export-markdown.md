...
- `export-disabled` keeps header title-row icon **まとめをコピー** / **ファイルに保存** disabled while status is not `ready`.

## How to get to it (user POV)

- Finish a meeting so `/meetings/<id>/document` is ready (preview: **おためし** → **おわる** → **はい、おわる**).
- Use the title-row icon buttons **まとめをコピー** and **ファイルに保存** (same `header` row as the meeting title). Hover shows those names.
...
- **Copy.** `page.locator("header").getByRole("button", { name: "まとめをコピー" }).click()`. Toast `コピーしました` is visible. `navigator.clipboard.readText()` contains `# <meeting title>` (preview title from the name field).
...
- Advice items also have a **コピー** button (question text). On the document page use `header` **まとめをコピー**, not an advice card.
