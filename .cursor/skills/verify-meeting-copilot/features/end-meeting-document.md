...
- **Confirm end.** Choose **おわる** then **はい、おわる**. `page.getByRole("button", { name: "はい、おわる" }).click()`. Text `まとめをつくっています` appears (preview waits ~720ms). Header **戻る** count is `0`. Then URL matches `/\/meetings\/.+\/document\?.*demo=1/`. A live generate that stays on making for 8s may show **つくるのをやめる** (not **戻る**).
