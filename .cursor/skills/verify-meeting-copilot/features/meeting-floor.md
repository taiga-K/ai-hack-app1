...
The meeting room is a floor, not a 1:1 chat. Desktop shows ささやき on the left and 話の地図 / 会議のメモ on the right. Preview seeds utterances, advice, and a growing map without a microphone.

## Sub-features

- `floor-sides` shows regions **こちら** and **むこう** with listening status in preview.
- `floor-whispers` shows complementary **こちらのささやき** with preview advice.
...
- On a narrow screen, use tablist **会議の表示** (`話の地図` / `メモ` / `ささやき`).
...
- **Whispers.** Complementary **こちらのささやき** is visible. Text `専門用語が説明なく使われています` and `❓ 専門用語の確認` are visible. Suggested question `『API連携でリアルタイム同期』は、今ある画面を見るだけですか？` is visible.
...
- **Splitter.** Drag `page.getByRole("separator", { name: "左右の幅を変える" })` about 90px to the right. Complementary **こちらのささやき** width changes by more than 20px. Map node `例外は宿題` stays inside the map region (±8px).
- **User-moved map.** Wheel the `.react-flow__pane` inside region **話の地図**. **ぜんぶ見る** appears. Click it. The button count returns to `0`.
- **Mobile panes.** Viewport `390x844`. Tablist **会議の表示**. Choose tab `/ささやき/`. Heading `ささやき` and the preview advice are visible. Map region still shows `今日の会議` (peek). Choose **話の地図** again.
- **Live empty floor.** From `/`, **はじめる**. Map empty copy `話しはじめると、ここにちいさな地図が育ちます`. Memos empty copy `まだ、だれも話していません`. Whispers empty copy `いまは、ささやくことがありません`. **ききはじめる** is visible.
